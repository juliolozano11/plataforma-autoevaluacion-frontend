...
import { useProgressPanel, useLevelsDistribution } from '@/hooks/use-reports';
...
export default function ReportsPage() {
  ...
  const { data: progress, isLoading: progressLoading } = useProgressPanel(selectedSectionId || undefined);
  const { data: distribution, isLoading: distributionLoading } = useLevelsDistribution();
  const { data: students } = useStudents(selectedCareer || undefined, selectedCourse || undefined);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-900">Panel de Progreso</h2>
          <select
      ...
      <Card className="p-6">
        ...
        {distributionLoading ? (
          ...
          <div className="space-y-4">
            {distribution?.sections?.map((sectionDist: any, index: number) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                <h3 className="font-medium text-gray-900 mb-3">
                  {sectionDist.sectionName || `Sección ${index + 1}`}
                </h3>
                <div className="grid grid-cols-5 gap-2">
                  {['muy_bajo', 'bajo', 'intermedio', 'alto', 'muy_alto'].map((level) => (
