...
import { EvaluationLevel, SectionResult } from '@/types';
...
const getValidLevel = (level: unknown): EvaluationLevel | null => {
  if (!level || typeof level !== 'string') return null;
  const validLevel = level as EvaluationLevel;
  return Object.values(EvaluationLevel).includes(validLevel)
    ? validLevel
    : null;
};
    ...
    <div className='max-w-4xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          ...
          <div>
            ...
            {(() => {
              ...
              return validLevel ? (
                <span
                  className={`inline-block px-4 py-2 rounded-full text-lg font-semibold mt-2 ${levelColors[validLevel]}`}
                >
      ...
      {report.sections && Array.isArray(report.sections) && (
        <div className='space-y-4'>
          ...
          {report.sections.map(
            (sectionResult: SectionResult, index: number) => {
              ...
              return (
                <Card key={index} className='p-6'>
                  <div className='flex items-center justify-between mb-4'>
                    ...
                    {validLevel && (
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${levelColors[validLevel]}`}
                      >
                        {levelLabels[validLevel]}
                      </span>
                    )}
